...
const {
  ...
  strapiAuthPassword,
  instagramSocialnetworkName,
  igBooleanQuestionNames,
} = require('./config');
...
const getQuestions = async (locale = 'ru') => {
  ...
  return rows.map((q) => {
    ...
    const isBooleanFallback =
      qa?.isBoolean == null && options.length === 0 && igBooleanQuestionNames.includes(name.toLowerCase());
    return {
      id: String(q?.id || ''),
      name,
      text: qa?.text || name || '',
...
const ensureInstagramLeadContact = async ({ leadId, userId }) => {
  const socialData = await requestAsService(
    ...
    `,
    { name: instagramSocialnetworkName },
  );

  const social = socialData?.socialnetworks?.data?.[0];
  if (!social?.id) throw new Error(`Socialnetwork not found: ${instagramSocialnetworkName}`);

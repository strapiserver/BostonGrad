...
const {
  ...
  strapiAuthPassword,
  facebookSocialnetworkName,
  fbBooleanQuestionNames,
} = require('./config');
...
const getLeadById = async (id) => {
  ...
  const contacts = Array.isArray(attrs?.lead_contacts?.data)
    ? attrs.lead_contacts.data.map((c) => ({
        ...
        socialnetworkName: c?.attributes?.socialnetwork?.data?.attributes?.name || '',
      }))
    : [];
...
const getQuestions = async (locale = 'ru') => {
  ...
  return rows.map((q) => {
    ...
    const options = Array.isArray(qa.options)
      ? qa.options.map((o) => o?.option).filter((v) => typeof v === 'string' && v.trim())
      : [];
    const name = String(qa?.name || '').trim();
    const isBooleanExplicit = qa?.isBoolean === true;
    const isBooleanFallback =
      qa?.isBoolean == null && options.length === 0 && fbBooleanQuestionNames.includes(name.toLowerCase());
    return {
      id: String(q?.id || ''),
      name,
      text: qa?.text || name || '',
...
const ensureFacebookLeadContact = async ({ leadId, userId }) => {
  const socialData = await requestAsService(
    ...
    `,
    { name: facebookSocialnetworkName },
  );

  const social = socialData?.socialnetworks?.data?.[0];
  if (!social?.id) throw new Error(`Socialnetwork not found: ${facebookSocialnetworkName}`);

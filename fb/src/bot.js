...
const { getLeadById, getQuestions, ensureFacebookLeadContact, createResponse } = require('./strapiClient');
const { sendText } = require('./metaApi');

const app = express();
app.use(express.json({ limit: '1mb' }));

const sessions = new Map(); // userId -> { leadId, questions, index, expiresAt, finalText }
const rateLimit = new Map(); // userId -> ts

const touchRateLimit = (userId) => {
  const now = Date.now();
  const last = rateLimit.get(userId) || 0;
  if (now - last < fbRateLimitMs) return false;
...
const parseAnswer = (q, textRaw) => {
  ...
  if (text.length > fbMaxAnswerLength) {
    return { ok: false, error: `Слишком длинный ответ (макс ${fbMaxAnswerLength}).` };
  }
...
const processMessage = async (userId, text) => {
  ...
  if (startMatch) {
    ...
    const existing = (lead.contacts || []).find(
      (c) => c.socialnetworkName.toLowerCase() === 'facebook' && c.user_id,
    );

    if (existing && existing.user_id !== String(userId)) {
      await sendText(userId, 'Этот код уже привязан к другому Facebook аккаунту.');
      return;
    }
    ...
    if (!existing) {
      await ensureFacebookLeadContact({ leadId: lead.id, userId: String(userId) });
    }
    ...
    sessions.set(String(userId), {
      ...
      questions,
      index: 0,
      expiresAt: Date.now() + fbSessionTtlSec * 1000,
  ...
  session.index += 1;
  session.expiresAt = Date.now() + fbSessionTtlSec * 1000;
...
const start = () => {
  app.listen(port, () => {
    console.log(`Facebook bot webhook listening on :${port}`);
  });
};

...
const getQuestions = async (locale = 'ru') => {
  ...
  return rows.map((q) => {
    ...
    const isBooleanFallback =
      qa?.isBoolean == null &&
      options.length === 0 &&
      tgBooleanQuestionNames.includes(name.toLowerCase());
...
const ensureTelegramLeadContact = async ({ leadId, tgUserId, username }) => {
  const socialData = await requestAsService(
    ...
    `,
    { name: telegramSocialnetworkName },
  );

  const social = socialData?.socialnetworks?.data?.[0];
  if (!social?.id) {
    throw new Error(`Socialnetwork not found: ${telegramSocialnetworkName}`);
  }
